...
download_datastream_proxies <- function(from = "1959-01-01", to = Sys.Date()) {
  
  cat(">> Downloading Datastream proxy variables", "from FRED & Yahoo...\n")
  ...
  fred_list <- purrr::compact(purrr::map(tickers, safe_fred, from, to))
